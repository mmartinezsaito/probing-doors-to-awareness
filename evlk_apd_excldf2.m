function [M1, M2, W, ni] = evlk_apd_excldf2(lkev_str, T, D, M, bls, ...
    rws, i_events, timelog, facvec, i, fi, facvec_excf2)
...
if ~is37
  ...
  nl = length(levs);
  for li = 1:nl
    ni(li) = sum(facvec == levs(li) & facvec_excf2~=2);
  end
  ...
  for j = 1:2
    ...
    nl{j} = length(levs{j});
    for li = 1:nl{j}
      ni(li,j) = sum(facvec{j} == levs{j}(li) & facvec_excf2{j}~=2);
    end
...
M1(:) = {0}; M2(:) = {0};
if ~is37
  for k = 1:ne, if facvec_excf2(k) ~= 2
  ...
  for j = 1:2
    for k = 1:ne{j}, if facvec_excf2{j}(k) ~= 2
